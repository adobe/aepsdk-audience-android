...
public final class Audience {
     ...
     *
     * <p>Visitor profile is saved persistently for easy access across multiple launches of your
     * app. If no audience signal has been submitted yet, null is returned.
     *
     * @param adobeCallback {@link AdobeCallback} instance which is invoked with the visitor's
     *     profile as a parameter; when an {@link AdobeCallbackWithError} is provided, an {@link
     *     AdobeError} can be returned in the eventuality of an unexpected error.
     * @see #signalWithData(Map, AdobeCallback)
     */
    public static void getVisitorProfile(
            @NonNull final AdobeCallback<Map<String, String>> adobeCallback) {
        if (adobeCallback == null) {
            Log.debug(
                    LOG_TAG,
                    LOG_SOURCE,
                    "Unexpected null callback, provide a callback to retrieve current"
                            + " visitorProfile.");
            return;
        }
                ...
                new AdobeCallbackWithError<Event>() {
                    ...
                    public void fail(final AdobeError adobeError) {
                        ...
                        final AdobeCallbackWithError<?> adobeCallbackWithError =
                                adobeCallback instanceof AdobeCallbackWithError
                    ...
                    public void call(final Event event) {
                        final Map<String, String> value =
                                DataReader.optStringMap(
                                        event.getEventData(), EventDataKeys.VISITOR_PROFILE, null);
     ...
     *
     * <p>Audience manager sends UUID in response to initial signal call. The UUID is persisted and
     * sent by SDK in all subsequent signal requests. If you are using Experience Cloud ID Service,
     * then Experience Cloud ID (MID) and other customerIDs for the same visitor are also sent in
     * each signal request.
     *
     * @param data traits data for the current visitor
     * @param adobeCallback {@link AdobeCallback} instance which is invoked with the visitor's
     *     profile as a parameter; when an {@link AdobeCallbackWithError} is provided, an {@link
     *     AdobeError} can be returned in the eventuality of an unexpected error or if the default
     *     timeout (5000ms) is met before the callback is returned with AAM profile.
     */
    ...
    public static void reset() {
        ...
        Log.debug(
                LOG_TAG,
                LOG_SOURCE,
                "Request to reset Audience Manager values for this device has been dispatched.");
    }
