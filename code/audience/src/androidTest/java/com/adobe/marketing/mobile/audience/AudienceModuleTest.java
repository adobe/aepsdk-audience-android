...
public class AudienceModuleTest extends SystemTest {
	...
	public void beforeEach() {
		...
		try {
			...
			eventHub.registerModuleWithCallback(AudienceExtension.class, new EventHub.RegisterModuleCallback() {
				@Override
	...
	@Test
	public void testSubmitSignal_when_NetworkHasUnRecoverableError_then_callBackCalledWithEmptyProfile() throws Exception {
		...
		TestableNetworkService.NetworkResponse response = new TestableNetworkService.NetworkResponse((String)null, 404, null);
	...
	public void testAudienceReset_shouldClearAllAudienceData() throws Exception {
		...
		aamSharedState.putStringMap(VISITOR_PROFILE_KEY, new HashMap<String, String>() {
			{
				put("key1", "val1");
			}
		});
		eventHub.createSharedState("com.adobe.module.audience", 0,
								   aamSharedState);
		...
		// Test
		eventHub.dispatch(new Event.Builder("AudienceRequestReset", EventType.AUDIENCEMANAGER,
		...
		assertNull(platformServices.getLocalStorageService().getDataStore(AAM_DATA_STORE).getString("AAMUserProfile", null));
	...
	@Test
	public void
	testSubmitSignal_then_LifecycleEventToAAM_then_ConfigUpdate_with_AAMForwardingEnabled_then_shouldSendRequestOnce()
	throws Exception {
		// setup expectations
		testableNetworkService.setExpectedCount(2);
		eventHub.ignoreAllStateChangeEvents();
	...
	@Test
	public void
	testSubmitSignal_then_LifecycleEventToAAM_then_IdentityUpdate_with_AAMForwardingEnabled_then_shouldSendRequestOnce()
	throws Exception {
		// setup expectations
		testableNetworkService.setExpectedCount(2);
		eventHub.ignoreAllStateChangeEvents();
		...
		eventHub.updateSharedState("com.adobe.module.identity", 0,
								   new EventData().putString("mid", "marketingCloudId").putString("blob", "blobValue").putString("locationhint",
										   "locationHintValue"));
	...
	@Test
	public void
	testSubmitSignal_then_LifecycleEventToAAM_then_ConfigUpdate_with_AAMForwardingDisabled_then_shouldSendRequestTwice()
	throws Exception {
		// setup expectations
		testableNetworkService.setExpectedCount(2);
		eventHub.ignoreAllStateChangeEvents();
	...
	@Test
	public void
	testLifecycleEvent_then_SubmitSignal_then_ConfigUpdate_with_then_shouldSendRequestTwice() throws Exception {
		// setup expectations
		testableNetworkService.setExpectedCount(2);
		eventHub.ignoreAllStateChangeEvents();
		eventHub.ignoreEvents(EventType.LIFECYCLE, EventSource.RESPONSE_CONTENT);
	...
	public void testSharedState_When_EverythingIsSet() throws Exception {
		...
		providedValidIdentityState();
		mockUUIDInPersistence();
		testableNetworkService.setDefaultResponse("{\"uuid\":\"19994521975870785742420741570375407533\", \"dests\":[{\"c\":\"http://someurl.com/forward\"}], \"stuff\":[{\"cv\":\"cv\",\"cn\":\"cn\"}]}");
	...
	public void testSubmitSignal_when_LocalStorageService_NotInitialized_ShouldNotCrash() throws Exception {
		...
		final HashMap < String, Object> audienceResponse = new HashMap <String, Object> ();
		testableNetworkService.setExpectedCount(2);
		testableNetworkService.setDefaultResponse("{\"uuid\":\"19994521975870785742420741570375407533\", \"dests\":[{\"c\":\"http://www.google.com\"}], \"stuff\":[{\"cv\":\"cv\",\"cn\":\"cn\"}]}");
		...
		assertTrue(loggingService.containsErrorLog("Audience Manager",
	...
	public void testSubmitSignal_when_PrivacyUnknown_Then_PrivacyChangesToOptIN_ShouldSendTwoHits() throws Exception {
		...
		testableNetworkService.setExpectedCount(2);
		testableNetworkService.setDefaultResponse("{\"uuid\":\"19994521975870785742420741570375407533\", \"stuff\":[{\"cv\":\"cv\",\"cn\":\"cn\"}]}");
	...
	public void testSubmitSignal_when_PrivacyUnknown_Then_PrivacyChangesToOptOUT_ShouldClearHits() throws Exception {
		...
		assertEquals(1, testableNetworkService.waitAndGetCount());
		assertEquals("The Network hit should have been the AAM optout hit!",
					 "https://server/demoptout.jpg?d_uuid=testUUID", testableNetworkService.getItem(0).url);
		waitForThreadsWithFailIfTimedOut(1000);
	...
	private void providedValidConfigurationState() {
		eventHub.createSharedState("com.adobe.module.configuration", 0,
								   new EventData().putString("audience.server", "server")
								   .putInteger("audience.timeout", 5).putString("global.privacy", "optedin").putBoolean("analytics.aamForwardingEnabled",
										   false));
	}
	...
	private void configureWithPrivacy(final String privacyStatus, final int eventNumber) {
		eventHub.createSharedState("com.adobe.module.configuration", eventNumber,
								   new EventData().putString("audience.server", "server")
								   .putInteger("audience.timeout", 5).putString("global.privacy",
										   privacyStatus).putBoolean("analytics.aamForwardingEnabled",
												   false));
	}
	...
	private void providedValidIdentityState() {
		eventHub.createSharedState("com.adobe.module.identity", 0,
								   new EventData().putString("mid", "marketingCloudId").putString("blob", "blobValue").putString("locationhint",
										   "locationHintValue"));
	}
	...
	private void mockVisitorProfileInPersistence() {
		...
		platformServices.getLocalStorageService().getDataStore(AAM_DATA_STORE).setMap("AAMUserProfile", visitorProfileMap);
	}
	...
	private Event createAudienceRequestIdentityEvent(final AdobeCallback<EventData> callback) {
		final Event event = new Event.Builder("AudienceGetDataProviderIds",
											  EventType.AUDIENCEMANAGER, EventSource.REQUEST_IDENTITY).build();
		eventHub.registerOneTimeListener(EventType.AUDIENCEMANAGER, EventSource.RESPONSE_IDENTITY,
	...
	private Event createAudienceRequestContentEventWithData(final HashMap<String, String> data,
			final AdobeCallback<HashMap<String, String>> callback) {
		final Event event = new Event.Builder("AudienceRequestContent",
											  EventType.AUDIENCEMANAGER, EventSource.REQUEST_CONTENT)
		.setData(new EventData().putStringMap(AudienceConstants.EventDataKeys.Audience.VISITOR_TRAITS, data))
		.build();

		if (callback != null) {
			eventHub.registerOneTimeListener(EventType.AUDIENCEMANAGER, EventSource.RESPONSE_CONTENT,
			event.getResponsePairID(), new Module.OneTimeListenerBlock() {
				@Override
				public void call(final Event e) {
					final EventData eventData = e.getData();
					callback.call((HashMap<String, String>) eventData.optStringMap(AudienceConstants.EventDataKeys.Audience.VISITOR_PROFILE,
								  null));
				}
			});
		}
	...
	private Event createLifecycleResponseEvent() {
		...
		final Event event = new Event.Builder("Lifecycle launch event", EventType.LIFECYCLE,
	...
	private Event createAAMRulesResponseEvent() {
		...
		final Event event = new Event.Builder("Rules Event", EventType.RULES_ENGINE,
											  EventSource.RESPONSE_CONTENT).setData(eventData).build();
		return event;
	}
	...
	void dispatchConfigurationResponseEvent(final MobilePrivacyStatus status) {
		...
		final Event event = new Event.Builder("Configuration Response Event", EventType.CONFIGURATION,
											  EventSource.RESPONSE_CONTENT).setData(config).build();
