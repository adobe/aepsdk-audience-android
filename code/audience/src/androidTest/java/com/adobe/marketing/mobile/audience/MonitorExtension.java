...
/**
 * A monitoring extension class aiding for assertion against dispatched events, shared state and XDM
 * shared state.
 */
public class MonitorExtension extends Extension {
    ...
    public void wildcardProcessor(final Event event) {
        if (AudienceTestConstants.EventType.MONITOR.equalsIgnoreCase(event.getType())) {
            if (AudienceTestConstants.EventSource.SHARED_STATE_REQUEST.equalsIgnoreCase(
                    event.getSource())) {
                processSharedStateRequest(event);
            } else if (AudienceTestConstants.EventSource.UNREGISTER.equalsIgnoreCase(
                    event.getSource())) {
    ...
    private void processSharedStateRequest(final Event event) {
        ...
        final String stateOwner =
                DataReader.optString(
                        eventData, AudienceTestConstants.EventDataKey.STATE_OWNER, null);
